...
uint32_t tmpfs_create(char *path, uint8_t type) {
  ...
  /*
    ...
    ./file.txt          ->   valid
    test/../../file.txt ->   invalid (if test exists then it is equivalent to
    ../file.txt)
  ...
  */
  ...
  if (!parent) {
    log_f(INFO, "tmpfs",
          "Failed to create %s, parent directory (%s) doesn't exist ", filename,
          parent_path);
    return 4;
  }

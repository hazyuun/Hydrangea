...
inline uint8_t ps2_ctrlr_cmd(uint8_t cmd) { return ps2_outb(PS2_CMD, cmd); }
...
ps2_device_driver_t ps2_device_drivers[] = {
    0,         &kbd_init, &mouse_init, &mouse_init,
    &kbd_init, &kbd_init, &mouse_init, &kbd_init,
};
...
uint8_t ps2_init() {
  ...
  log_info(NICE_BLUE_0, "PS/2", "The controller has %s",
           dual ? "2 channels" : "1 channel");
  ...
  for (uint8_t i = 0; i < 2; i++) {
    ...
    uint8_t ack_success =
        (A0 == PS2_DEV_ACK && A1 == PS2_DEV_RESET_ACK) ||

...
void BST_add(BST_node_t* root, void* key, comparator_t cmp){
     ...
     if(cmp(key, root->key) == 1){
          if(root->right) BST_add(root->right, key, cmp);
          else {
          ...
          if(root->left) BST_add(root->left, key, cmp);
          else {
...
BST_node_t* BST_remove(BST_node_t* root, void* key, comparator_t cmp){
     if(!root) return root;
     if(cmp(key, root->key) == 1){
          if(root->right) root->right = BST_remove(root->right, key, cmp);
          else return root;
     }else if(cmp(key, root->key) == -1){
          if(root->left) root->left = BST_remove(root->left, key, cmp);
          else return root;
     }else{
...
BST_node_t* BST_find(BST_node_t* root, void* key, comparator_t cmp){
     if(!root) return NULL;
     if(cmp(key, root->key) == 1){
          if(root->right) return BST_find(root->right, key, cmp);
          else return NULL;
     }else if(cmp(key, root->key) == -1){
          if(root->left) return BST_find(root->left, key, cmp);
          else return NULL;
     }else return root;
}
...
void BST_show(BST_node_t* root, int lvl){
     ...
     if(root->right){
          i = lvl;
          while(i-->0) printk("|   ");
          printk("R-- "); BST_show(root->right, lvl+1);
     }
     if(root->left){
          i = lvl;
          while(i-->0) printk("|   ");
          printk("L-- "); BST_show(root->left, lvl+1);
     }

...
static uint32_t elf_load_segments(uint8_t *buffer) {
  ...
  /*
  log_info(NICE_GREEN_0, "ELF", "Program header table offset : %d Bytes",
  eh->ph_offset); log_info(NICE_GREEN_0, "ELF", "Program header size         :
  %d Bytes", eh->ph_table_entry_size); log_info(NICE_GREEN_0, "ELF", "Program
  header count        : %d Bytes", eh->ph_table_entry_count);
  */
  elf_ph_header_t *ph_tbl = (elf_ph_header_t *)(buffer + eh->ph_offset);

  for (uint16_t n = 0; n < eh->ph_table_entry_count; n++) {
    if (ph_tbl[n].type == ELF_PH_SEGTYPE_NULL)
      continue;
    if (ph_tbl[n].type == ELF_PH_SEGTYPE_LOAD) {
      // log_info(NICE_GREEN_0, "ELF", "Loading segment into vaddr=%d",
      // ph_tbl[n].vaddr);
      ...
      memcpy((void *)(ph_tbl[n].vaddr), buffer + ph_tbl[n].offset,
             ph_tbl[n].mem_size);
      if (ph_tbl[n].file_size < ph_tbl[n].mem_size)
        memset((void *)(ph_tbl[n].vaddr + ph_tbl[n].file_size), 0,
               ph_tbl[n].mem_size - ph_tbl[n].file_size);
    }
  }

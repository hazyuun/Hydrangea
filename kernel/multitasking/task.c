...
task_t *ktask_create(char *name, uint32_t ppid, void (*entry)(void *),
  ...
  *(--stack) = 0x0;
  ...
  /* there is no ss, esp because there is no priviliage change (ring 0 to ring
   * 0) */
  *(--stack) = 0x200;
  *(--stack) = 0x8;
  ...
  *(--stack) = 0x200; /* eflags */
  ...
  // log_f(INFO, "ktask_create", "Created kernel task (PID %d) : %s", task->pid,
  // name); log_f(INFO, "ktask_create", "cwd : %s",
  // vfs_abs_path_to(task->cwd_node));

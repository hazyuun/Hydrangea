...
void tty_cur_stp() { tty_cur_mov(c_tty_state.column, c_tty_state.row); }

void tty_scroll() {
  for (size_t i = 0; i < VGA_HEIGHT; i++) {
    for (size_t j = 0; j < VGA_WIDTH; j++) {
      tty_frame_buffer[i * VGA_WIDTH + j] =
          tty_frame_buffer[(i + 1) * VGA_WIDTH + j];
    }
  }
}
...
void tty_putat(unsigned char c, size_t x, size_t y) {
  tty_frame_buffer[x + y * (size_t)(VGA_WIDTH)] =
      vga_char(c, c_tty_state.color);
  tty_cur_stp();
}
